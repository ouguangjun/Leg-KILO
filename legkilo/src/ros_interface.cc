...
void RosInterface::imuLoop(){
    ...
    ros::CallbackQueue queue;
    nh.setCallbackQueue(&queue);
    this->sub_imu_raw_ = nh.subscribe(
        options::kImuTopic, 10000, &RosInterface::imuCallBack, this);
...
void RosInterface::kinematicImuLoop(){
    ...
    ros::CallbackQueue queue;
    nh.setCallbackQueue(&queue);
    this->sub_kinematic_raw_ = nh.subscribe(
        options::kKinematicTopic, 10000, &RosInterface::kinematicImuCallBack, this);
...
bool RosInterface::predictUpdatePoint(double current_time, size_t idx_i, size_t idx_j){
    ...
    for(size_t i = 0; i < points_size; ++i){
        ...
        if(iter != map_manager_->voxel_map_.end()){
            ...
            if (!is_success)
            {
                ...
                if (iter_near != map_manager_->voxel_map_.end()) { map_manager_->build_single_residual(cur_pt_var, iter_near->second, 0, is_success, prob, single_ptpl); }
            }
            if(is_success){
                ++success_pts_size;
    ...
    if(eskf_update){
        ...
        for(size_t k = 0; k < effect_num; ++k){
            ...
            obs_shared.pt_h.row(k) <<  crossmat_rotT_u(0), crossmat_rotT_u(1), crossmat_rotT_u(2), ptpl_list[k].normal_(0), ptpl_list[k].normal_(1), ptpl_list[k].normal_(2);
            ...
            M3D var;
            var = eskf_->state().rot_ * ext_rot_ * ptpl_list[k].body_cov_ * ext_rot_.transpose() * eskf_->state().rot_.transpose();
            double single_l = J_nq * ptpl_list[k].plane_var_ * J_nq.transpose();
            obs_shared.pt_R(k) =  eskf_->config().lidar_point_meas_ratio *  (single_l + ptpl_list[k].normal_.transpose() * var *ptpl_list[k].normal_);
...
bool RosInterface::predictUpdateImu(const sensor_msgs::ImuPtr& imu){
    ...
    obs_shared.ki_R << eskf_->config().imu_acc_meas_noise, eskf_->config().imu_acc_meas_noise, eskf_->config().imu_acc_z_meas_noise, 
                                            eskf_->config().imu_gyr_meas_noise, eskf_->config().imu_gyr_meas_noise, eskf_->config().imu_gyr_meas_noise;
...
bool RosInterface::predictUpdateKinImu(const common::KinImuMeas& kin_imu){
    ...
    obs_shared.ki_R.block<6, 1>(0, 0) << eskf_->config().imu_acc_meas_noise, eskf_->config().imu_acc_meas_noise, eskf_->config().imu_acc_z_meas_noise, 
                                            eskf_->config().imu_gyr_meas_noise, eskf_->config().imu_gyr_meas_noise, eskf_->config().imu_gyr_meas_noise;
    ...
    for(int i = 0; i < 4; ++i){
        if(kin_imu.contact_[i]){
            ...
            obs_shared.ki_R.block<3, 1>(6 + 3 * idx, 0)  << eskf_->config().kin_meas_noise, eskf_->config().kin_meas_noise, eskf_->config().kin_meas_noise;
            idx++;
        }
    }
...
void RosInterface::run(){
    ...
    if(cloud_raw_->points.empty() || 
        (options::kImuUse && imus.empty()) || 
    ...
    if(init_flag_){
        ...
        LOG(INFO) << "Gravity is initialized to "
                               << std::fixed << std::setprecision(3)
                               << gravity(0) << " "<< gravity(1) << " " << gravity(2); 
        LOG(INFO) << "IMU bw is initialized to "
                               << bw(0) << " " << bw(1) << " " << bw(2);
    ...
    LOG(INFO) << "pcl raw size:  " << cloud_raw_->points.size() << "  pcl down size: " << cloud_down_body_->points.size();
