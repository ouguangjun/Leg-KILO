...
RosInterface::RosInterface(ros::NodeHandle &nh) : nh_(nh) {
  ...
  pub_pointcloud_world_ =
      nh.advertise<sensor_msgs::PointCloud2>("/cloud_registered", 10000);
  pub_pointcloud_body_ =
      nh.advertise<sensor_msgs::PointCloud2>("/cloud_registered_body", 10000);
...
bool RosInterface::initParamAndReset(const std::string &config_file) {
  ...
  if (options::kImuUse) {
    options::kImuTopic = yaml_helper.get<std::string>("imu_topic");
  }
  ...
  eskf_config.imu_acc_process_cov =
      yaml_helper.get<double>("imu_acc_process_cov");
  eskf_config.imu_gyr_process_cov =
      yaml_helper.get<double>("imu_gyr_process_cov");
  eskf_config.acc_bias_process_cov =
      yaml_helper.get<double>("acc_bias_process_cov");
  eskf_config.gyr_bias_process_cov =
      yaml_helper.get<double>("gyr_bias_process_cov");
  eskf_config.kin_bias_process_cov =
      yaml_helper.get<double>("kin_bias_process_cov");
  eskf_config.contact_process_cov =
      yaml_helper.get<double>("contact_process_cov");

  eskf_config.imu_acc_meas_noise =
      yaml_helper.get<double>("imu_acc_meas_noise");
  eskf_config.imu_acc_z_meas_noise =
      yaml_helper.get<double>("imu_acc_z_meas_noise");
  eskf_config.imu_gyr_meas_noise =
      yaml_helper.get<double>("imu_gyr_meas_noise");
  ...
  eskf_config.contact_meas_noise =
      yaml_helper.get<double>("contact_meas_noise");
  eskf_config.lidar_point_meas_ratio =
      yaml_helper.get<double>("lidar_point_meas_ratio");
  ...
  kinematics_config.leg_calf_length =
      yaml_helper.get<double>("leg_calf_length");
  kinematics_config.leg_thigh_length =
      yaml_helper.get<double>("leg_thigh_length");
  kinematics_config.leg_thigh_offset =
      yaml_helper.get<double>("leg_thigh_offset");
  kinematics_config.contact_force_threshold_up =
      yaml_helper.get<double>("contact_force_threshold_up");
  kinematics_config.contact_force_threshold_down =
      yaml_helper.get<double>("contact_force_threshold_down");
  ...
  lidar_process_config.point_stamp_correct_ =
      yaml_helper.get<bool>("point_stamp_correct", false);
  lidar_process_config.lidar_type_ =
      static_cast<common::LidarType>(yaml_helper.get<int>("lidar_type"));
  ...
  voxel_grid_.setLeafSize(voxel_grid_resolution, voxel_grid_resolution,
  ...
  voxel_map_config.planner_threshold_ =
      yaml_helper.get<double>("min_eigen_value");
  ...
  voxel_map_config.layer_init_num_ =
      yaml_helper.get<std::vector<int>>("layer_init_num");
  ...
  std::vector<double> ext_t =
      yaml_helper.get<std::vector<double>>("extrinsic_T");
  std::vector<double> ext_R =
      yaml_helper.get<std::vector<double>>("extrinsic_R");
...
void RosInterface::rosInit(const std::string &config_file) {
  ...
  if (options::kKinAndImuUse) {
    this->subscribeKinematicImu();
  }
}

void RosInterface::subscribeLidar() {
  this->lidar_thread_ = std::unique_ptr<std::thread>(
      new std::thread(&RosInterface::lidarLoop, this));
  THREAD_SLEEP(100);
}

void RosInterface::subscribeImu() {
  this->imu_thread_ = std::unique_ptr<std::thread>(
      new std::thread(&RosInterface::imuLoop, this));
  THREAD_SLEEP(100);
}

void RosInterface::subscribeKinematicImu() {
  this->kinematic_thread_ = std::unique_ptr<std::thread>(
      new std::thread(&RosInterface::kinematicImuLoop, this));
  THREAD_SLEEP(100);
}
...
void RosInterface::lidarLoop() {
  ...
  this->sub_lidar_raw_ = nh.subscribe<sensor_msgs::PointCloud2>(
      options::kLidarTopic, 1000, &RosInterface::lidarCallBack, this);

  while (ros::ok() && !options::FLAG_EXIT) {
...
void RosInterface::imuLoop() {
  ...
  nh.setCallbackQueue(&queue);
  this->sub_imu_raw_ =
      nh.subscribe(options::kImuTopic, 10000, &RosInterface::imuCallBack, this);
...
void RosInterface::kinematicImuLoop() {
  ...
  this->sub_kinematic_raw_ =
      nh.subscribe(options::kKinematicTopic, 10000,
                   &RosInterface::kinematicImuCallBack, this);
...
void RosInterface::lidarCallBack(
...
void RosInterface::kinematicImuCallBack(
    const unitree_legged_msgs::HighState::ConstPtr &msg) {
  static unitree_legged_msgs::HighState last_highstate_msg;
  unitree_legged_msgs::HighStatePtr highstate_msg(
      new unitree_legged_msgs::HighState(*msg));

  if (options::kRedundancy) {
    if (highstate_msg->imu.accelerometer[2] ==
            last_highstate_msg.imu.accelerometer[2] &&
        highstate_msg->imu.gyroscope[2] ==
            last_highstate_msg.imu.gyroscope[2]) {
...
bool RosInterface::syncPackage() {
  ...
  if (options::kKinAndImuUse) {
    if (lidar_cache_.empty() || kin_imu_cache_.empty())
      return false;
    ...
    while ((!kin_imu_cache_.empty()) && (kin_imu_time < lidar_end_time_)) {
      ...
      if (kin_imu_time > lidar_end_time_)
        break;
...
inline void RosInterface::pointLidarToImu(const PointType &point_lidar,
                                          PointType &point_imu) {
...
inline void RosInterface::pointLidarToWorld(const PointType &point_lidar,
...
void RosInterface::cloudLidarToWorld(const CloudPtr &cloud_lidar,
...
void RosInterface::publishOdomTFPath(double end_time) {
  ...
  // tf
  transform_.setOrigin(tf::Vector3(odom_world_.pose.pose.position.x,
                                   odom_world_.pose.pose.position.y,
  ...
  br_.sendTransform(tf::StampedTransform(transform_, odom_world_.header.stamp,
                                         "camera_init", "body"));
...
bool RosInterface::predictUpdatePoint(double current_time, size_t idx_i,
                                      size_t idx_j) {
  /* 1. predict state*/
  ...
  for (size_t i = 0; i < points_size; ++i) {
    ...
    cur_pt_var.point_w =
        eskf_->state().rot_ * cur_pt_var.point_i + eskf_->state().pos_;
    ...
    cur_pt_var.var = rot_extR * cur_pt_var.body_var * rot_extR.transpose() +
                     rot_crossmat * eskf_->cov().block<3, 3>(0, 0) *
                         rot_crossmat.transpose() +
    ...
    for (int j = 0; j < 3; j++) {
      loc_xyz[j] =
          cur_pt_var.point_w[j] / map_manager_->config_setting_.max_voxel_size_;
      if (loc_xyz[j] < 0) {
        loc_xyz[j] -= 1.0;
      }
    }
    ...
    if (iter != map_manager_->voxel_map_.end()) {
      ...
      double prob = 0;
      map_manager_->build_single_residual(cur_pt_var, current_octo, 0,
      ...
      if (!is_success) {
        ...
        if (iter_near != map_manager_->voxel_map_.end()) {
          map_manager_->build_single_residual(cur_pt_var, iter_near->second, 0,
                                              is_success, prob, single_ptpl);
        }
      }
      if (is_success) {
  ...
  if (eskf_update) {
    ...
    for (size_t k = 0; k < effect_num; ++k) {
      V3D crossmat_rotT_u = ptpl_list[k].point_crossmat_ *
                            eskf_->state().rot_.transpose() *
                            ptpl_list[k].normal_;
      obs_shared.pt_h.row(k) << crossmat_rotT_u(0), crossmat_rotT_u(1),
          crossmat_rotT_u(2), ptpl_list[k].normal_(0), ptpl_list[k].normal_(1),
          ptpl_list[k].normal_(2);
      ...
      M3D var;
      var = eskf_->state().rot_ * ext_rot_ * ptpl_list[k].body_cov_ *
            ext_rot_.transpose() * eskf_->state().rot_.transpose();
      double single_l = J_nq * ptpl_list[k].plane_var_ * J_nq.transpose();
      obs_shared.pt_R(k) = eskf_->config().lidar_point_meas_ratio *
                           (single_l + ptpl_list[k].normal_.transpose() * var *
                                           ptpl_list[k].normal_);
    }
  ...
  if (eskf_update) {
    for (size_t i = 0; i < points_size; ++i) {
      pv_list[i].point_w =
          eskf_->state().rot_ * pv_list[i].point_i + eskf_->state().pos_;
...
bool RosInterface::predictUpdateImu(const sensor_msgs::ImuPtr &imu) {
  ...
  Vec3D imu_acc(imu->linear_acceleration.x, imu->linear_acceleration.y,
                imu->linear_acceleration.z);
  Vec3D imu_gyr(imu->angular_velocity.x, imu->angular_velocity.y,
                imu->angular_velocity.z);
  obs_shared.ki_z.block<3, 1>(0, 0) = (gravity_ / acc_norm_) * imu_acc -
                                      eskf_->state().imu_a_ -
                                      eskf_->state().ba_;
  obs_shared.ki_z.block<3, 1>(3, 0) =
      imu_gyr - eskf_->state().imu_w_ - eskf_->state().bw_;

  obs_shared.ki_R << eskf_->config().imu_acc_meas_noise,
      eskf_->config().imu_acc_meas_noise, eskf_->config().imu_acc_z_meas_noise,
      eskf_->config().imu_gyr_meas_noise, eskf_->config().imu_gyr_meas_noise,
...
bool RosInterface::predictUpdateKinImu(const common::KinImuMeas &kin_imu) {
  ...
  obs_shared.ki_R.block<6, 1>(0, 0) << eskf_->config().imu_acc_meas_noise,
      eskf_->config().imu_acc_meas_noise, eskf_->config().imu_acc_z_meas_noise,
      eskf_->config().imu_gyr_meas_noise, eskf_->config().imu_gyr_meas_noise,
      eskf_->config().imu_gyr_meas_noise;
  ...
  for (int i = 0; i < 4; ++i) {
    if (kin_imu.contact_[i]) {
      Vec3D foot_pos(kin_imu.foot_pos_[i][0], kin_imu.foot_pos_[i][1],
                     kin_imu.foot_pos_[i][2]);
      Vec3D foot_vel(kin_imu.foot_vel_[i][0], kin_imu.foot_vel_[i][1],
                     kin_imu.foot_vel_[i][2]);

      Vec3D w_skew_pos_vel = w_skew * foot_pos + foot_vel;

      obs_shared.ki_h.block<3, 3>(6 + 3 * idx, 0) =
          -eskf_->state().rot_ * SKEW_SYM_MATRIX(w_skew_pos_vel);
      obs_shared.ki_h.block<3, 3>(6 + 3 * idx, 6) = Mat3D::Identity();
      obs_shared.ki_h.block<3, 3>(6 + 3 * idx, 21) =
          -eskf_->state().rot_ * SKEW_SYM_MATRIX(foot_pos);

      obs_shared.ki_z.block<3, 1>(6 + 3 * idx, 0) =
          -eskf_->state().vel_ - eskf_->state().rot_ * w_skew_pos_vel;

      obs_shared.ki_R.block<3, 1>(6 + 3 * idx, 0)
          << eskf_->config().kin_meas_noise,
...
void RosInterface::run() {
  ...
  if (cloud_raw_->points.empty() || (options::kImuUse && imus.empty()) ||
      (!options::kImuUse && kin_imus.empty())) {
  ...
  if (init_flag_) {
    ...
    map_manager_->BuildVoxelMap(eskf_->state().rot_,
                                eskf_->cov().block<3, 3>(0, 0),
                                eskf_->cov().block<3, 3>(3, 3));

    auto gravity = eskf_->state().grav_;
    auto bw = eskf_->state().bw_;
    LOG(INFO) << "Initialization is finished";
    LOG(INFO) << "Gravity is initialized to " << std::fixed
              << std::setprecision(3) << gravity(0) << " " << gravity(1) << " "
              << gravity(2);
    LOG(INFO) << "IMU bw is initialized to " << bw(0) << " " << bw(1) << " "
              << bw(2);
  ...
  Timer::measure("State predict/update & Map update", [&, this]() {
    /* pointcloud sort*/
    std::sort(cloud_down_body_->points.begin(), cloud_down_body_->points.end(),
              time_list);
    ...
    while (idx_i < pts_size) {
      ...
      if (options::kImuUse) {
        while (!imus.empty() &&
               imus.front()->header.stamp.toSec() < cur_point_time) {
          /* process imu*/
          this->predictUpdateImu(imus.front());
          imus.pop_front();
        }
      } else {
        while (!kin_imus.empty() &&
               kin_imus.front().time_stamp_ < cur_point_time) {
          /* process kin imu*/
  ...
  // static int countt = 0;
  // if(!(countt++ % 10)) std::cout << eskf_->cov().block<6, 6>(0, 0) <<
  // std::endl << std::endl;

  LOG(INFO) << "pcl raw size:  " << cloud_raw_->points.size()
            << "  pcl down size: " << cloud_down_body_->points.size();
  LOG(INFO) << "useful pcl percent :  "
            << 100 * ((double)(success_pts_size) /
                      cloud_down_body_->points.size())
            << " %";
