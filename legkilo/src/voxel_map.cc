...
// {
//   nh.param<bool>("publish/pub_plane_en", voxel_config.is_pub_plane_map_,
//   false);
...
//   false); nh.param<int>("local_map/half_map_size",
//   voxel_config.half_map_size, 100);
//   nh.param<double>("local_map/sliding_thresh", voxel_config.sliding_thresh,
//   8);
// }
...
void VoxelOctoTree::init_plane(const std::vector<pointWithVar> &points,
  ...
  // if(evalsMid >= 3){
  //   std::cout << static_cast<int>(evalsMin) << " " <<
  //   static_cast<int>(evalsMax) << std::endl; throw std::runtime_error("wrong
  //   evals mid index");
  // }
...
void VoxelMapManager::BuildVoxelMap(const Eigen::Matrix3d rot,
...
// void VoxelMapManager::BuildResidualListOMP(std::vector<pointWithVar>
// &pv_list, std::vector<PointToPlane> &ptpl_list)
// {
//   int max_layer = config_setting_.max_layer_;
...
//     }
//     VOXEL_LOCATION position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1],
//     (int64_t)loc_xyz[2]); auto iter = voxel_map_.find(position); if (iter !=
//     voxel_map_.end())
//     {
...
//       double prob = 0;
//       build_single_residual(pv, current_octo, 0, is_success, prob,
//       single_ptpl); if (!is_success)
//       {
//         VOXEL_LOCATION near_position = position;
//         if (loc_xyz[0] > (current_octo->voxel_center_[0] +
//         current_octo->quater_length_)) { near_position.x = near_position.x +
//         1; } else if (loc_xyz[0] < (current_octo->voxel_center_[0] -
//         current_octo->quater_length_)) { near_position.x = near_position.x -
//         1; } if (loc_xyz[1] > (current_octo->voxel_center_[1] +
//         current_octo->quater_length_)) { near_position.y = near_position.y +
//         1; } else if (loc_xyz[1] < (current_octo->voxel_center_[1] -
//         current_octo->quater_length_)) { near_position.y = near_position.y -
//         1; } if (loc_xyz[2] > (current_octo->voxel_center_[2] +
//         current_octo->quater_length_)) { near_position.z = near_position.z +
//         1; } else if (loc_xyz[2] < (current_octo->voxel_center_[2] -
//         current_octo->quater_length_)) { near_position.z = near_position.z -
//         1; } auto iter_near = voxel_map_.find(near_position); if (iter_near
//         != voxel_map_.end()) { build_single_residual(pv, iter_near->second,
//         0, is_success, prob, single_ptpl); }
//       }
//       if (is_success)
//       {
//         mylock.lock();
...
//     feats_down_body_->points[i].y, feats_down_body_->points[i].z); if
//     (point_this[2] == 0) { point_this[2] = 0.001; } M3D var;
//     calcBodyCov(point_this, config_setting_.dept_err_,
//     config_setting_.beam_err_, var); body_cov_list_.push_back(var);
...
//     pcl::PointCloud<pcl::PointXYZI>); TransformLidar(state_.rot_end,
//     state_.pos_end, feats_down_body_, world_lidar); M3D rot_var =
//     state_.cov.block<3, 3>(0, 0); M3D t_var = state_.cov.block<3, 3>(3, 3);
//     for (size_t i = 0; i < feats_down_body_->size(); i++)
//     {
...
//          << " effective feature num: " << effct_feat_num_ << " average
//          residual: " << total_residual / effct_feat_num_ << endl;

//     /*** Computation of Measuremnt Jacobian matrix H and measurents
//     covarience
//      * ***/
...
//       state_propagat.pos_end; Eigen::Matrix<double, 1, 6> J_nq; J_nq.block<1,
//       3>(0, 0) = point_world - ptpl_list_[i].center_; J_nq.block<1, 3>(0, 3)
...
//       // ptpl_list_[i].body_cov_ = ptpl_list_[i].body_cov_ * (1.0 /
//       cos_theta) * (1.0 / cos_theta);
...
//       state_propagat.cov.block<3, 3>(0, 0) * (-point_crossmat).transpose();
...
//       R_inv(i) = 1.0 / (0.001 + sigma_l + ptpl_list_[i].normal_.transpose() *
//       var * ptpl_list_[i].normal_);
//       // R_inv(i) = 1.0 / (sigma_l + ptpl_list_[i].normal_.transpose() * var
//       * ptpl_list_[i].normal_);

//       /*** calculate the Measuremnt Jacobian matrix H ***/
//       V3D A(point_crossmat * state_.rot_end.transpose() *
//       ptpl_list_[i].normal_); Hsub.row(i) << VEC_FROM_ARRAY(A),
//       ptpl_list_[i].normal_[0], ptpl_list_[i].normal_[1],
//       ptpl_list_[i].normal_[2]; Hsub_T_R_inv.col(i) << A[0] * R_inv(i), A[1]
//       * R_inv(i), A[2] * R_inv(i), ptpl_list_[i].normal_[0] * R_inv(i),
//           ptpl_list_[i].normal_[1] * R_inv(i), ptpl_list_[i].normal_[2] *
//           R_inv(i);
...
//     G.block<DIM_STATE, 6>(0, 0) = K_1.block<DIM_STATE, 6>(0, 0) *
//     H_T_H.block<6, 6>(0, 0); auto vec = state_propagat - state_;
//     VD(DIM_STATE)
//     solution = K_1.block<DIM_STATE, 6>(0, 0) * HTz + vec.block<DIM_STATE,
//     1>(0, 0) - G.block<DIM_STATE, 6>(0, 0) * vec.block<6, 1>(0, 0); int
//     minRow, minCol; state_ += solution; auto rot_add = solution.block<3,
//     1>(0, 0); auto t_add = solution.block<3, 1>(3, 0); if ((rot_add.norm()
//     * 57.3 < 0.01) && (t_add.norm() * 100 < 0.015)) { flg_EKF_converged =
//     true; } V3D euler_cur = state_.rot_end.eulerAngles(2, 1, 0);

//     /*** Rematch Judgement ***/

//     if (flg_EKF_converged || ((rematch_num == 0) && (iterCount ==
//     (config_setting_.max_iterations_ - 2)))) { rematch_num++; }

//     /*** Convergence Judgements and Covariance Update ***/
//     if (!EKF_stop_flg && (rematch_num >= 2 || (iterCount ==
//     config_setting_.max_iterations_ - 1)))
//     {
//       /*** Covariance Update ***/
//       // _state.cov = (I_STATE - G) * _state.cov;
//       state_.cov.block<DIM_STATE, DIM_STATE>(0, 0) =
//           (I_STATE.block<DIM_STATE, DIM_STATE>(0, 0) - G.block<DIM_STATE,
//           DIM_STATE>(0, 0)) * state_.cov.block<DIM_STATE, DIM_STATE>(0, 0);
//       // total_distance += (_state.pos_end - position_last).norm();
//       position_last_ = state_.pos_end;
//       geoQuat_ = tf::createQuaternionMsgFromRollPitchYaw(euler_cur(0),
//       euler_cur(1), euler_cur(2));
...
//   scan_count + build_residual_time / scan_count;
//   // ave_ekf_time = ave_ekf_time * (scan_count - 1) / scan_count + ekf_time /
//   scan_count;

//   // cout << "[ Mapping ] ekf_time: " << ekf_time << "s, build_residual_time:
//   " << build_residual_time << "s" << endl;
//   // cout << "[ Mapping ] ave_ekf_time: " << ave_ekf_time << "s,
//   ave_build_residual_time: " << ave_build_residual_time << "s" << endl;
// }

void VoxelMapManager::build_single_residual(pointWithVar &pv,
                                            const VoxelOctoTree *current_octo,
                                            const int current_layer,
                                            bool &is_success, double &prob,
                                            PointToPlane &single_ptpl) {
  int max_layer = config_setting_.max_layer_;
  double sigma_num = config_setting_.sigma_num_;
    ...
    if (current_layer < max_layer) {
      for (size_t leafnum = 0; leafnum < 8; leafnum++) {
        if (current_octo->leaves_[leafnum] != nullptr) {
          ...
          build_single_residual(pv, leaf_octo, current_layer + 1, is_success,
                                prob, single_ptpl);
        }
      }
      return;
...
bool VoxelMapManager::mapSliding() {
  if ((position_last_ - last_slide_position).norm() <
    ...
    // std::cout<<RED<<"[DEBUG]: Last sliding length "<<(position_last_ -
    // last_slide_position).norm()<<RESET<<"\n";
    return false;
  }
  ...
  // std::cout<<RED<<"[DEBUG]: Map sliding using "<<t_sliding_end -
  // t_sliding_start<<" secs"<<RESET<<"\n";
  return true;
}
