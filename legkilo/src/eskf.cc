...
void ESKF::initProcessCovQ() {
  Q_.setZero();
  Q_.block<3, 3>(6, 6).diagonal() << config_.vel_process_cov,
      config_.vel_process_cov, config_.vel_process_cov;
  Q_.block<3, 3>(9, 9).diagonal() << config_.acc_bias_process_cov,
      config_.acc_bias_process_cov, config_.acc_bias_process_cov;
  Q_.block<3, 3>(12, 12).diagonal() << config_.gyr_bias_process_cov,
      config_.gyr_bias_process_cov, config_.gyr_bias_process_cov;
  Q_.block<3, 3>(18, 18).diagonal() << config_.imu_acc_process_cov,
      config_.imu_acc_process_cov, config_.imu_acc_process_cov;
  Q_.block<3, 3>(21, 21).diagonal() << config_.imu_gyr_process_cov,
      config_.imu_gyr_process_cov, config_.imu_gyr_process_cov;
  Q_.block<3, 3>(24, 24).diagonal() << config_.kin_bias_process_cov,
      config_.kin_bias_process_cov, config_.kin_bias_process_cov;
  Q_.block<3, 3>(27, 27).diagonal() << config_.contact_process_cov,
      config_.contact_process_cov, config_.contact_process_cov;
}
...
void ESKF::updateByPoints(ObsShared &obs_shared) {
  ...
  if (dof_measurements == 1) {
    Eigen::Matrix<double, DIM_STATE, 1> PHT =
        cov_.block<DIM_STATE, 6>(0, 0) * h.transpose();
  ...
  // else{
  //     Eigen::MatrixXd H_T_R_inv = h.transpose() *
  //     r.cwiseInverse().asDiagonal(); Eigen::MatrixXd P_temp = H_T_R_inv * h +
  //     cov_.inverse().block<6, 6>(0, 0); Eigen::MatrixXd K =
  //     P_temp.inverse().block<DIM_STATE, 6>(0, 0) * H_T_R_inv; StateVec
  //     delta_x = K * z; state_ += delta_x; cov_ = cov_ - K * h * cov_.block<6,
  //     DIM_STATE>(0, 0);
  // }
}
...
void ESKF::updateByImu(ObsShared &obs_shared) {
  // simplify by matlab
  Eigen::Matrix<double, DIM_STATE, 6> PHT =
      cov_.block<DIM_STATE, 6>(0, 9) + cov_.block<DIM_STATE, 6>(0, 18);
  Eigen::Matrix<double, 6, DIM_STATE> HP =
      cov_.block<6, DIM_STATE>(9, 0) + cov_.block<6, DIM_STATE>(18, 0);
  Eigen::Matrix<double, 6, 6> HPHT =
      PHT.block<6, 6>(9, 0) + PHT.block<6, 6>(18, 0);
