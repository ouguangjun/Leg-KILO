...
void ESKF::initProcessCovQ(){
    ...
    Q_.block<3, 3>(9, 9).diagonal() << config_.acc_bias_process_cov, config_.acc_bias_process_cov, config_.acc_bias_process_cov;
    Q_.block<3, 3>(12, 12).diagonal() << config_.gyr_bias_process_cov, config_.gyr_bias_process_cov, config_.gyr_bias_process_cov;
    Q_.block<3, 3>(18, 18).diagonal() << config_.imu_acc_process_cov, config_.imu_acc_process_cov, config_.imu_acc_process_cov;
    Q_.block<3, 3>(21, 21).diagonal() << config_.imu_gyr_process_cov, config_.imu_gyr_process_cov, config_.imu_gyr_process_cov;
    Q_.block<3, 3>(24, 24).diagonal() << config_.kin_bias_process_cov, config_.kin_bias_process_cov, config_.kin_bias_process_cov;
    Q_.block<3, 3>(27, 27).diagonal() << config_.contact_process_cov, config_.contact_process_cov, config_.contact_process_cov;
}
