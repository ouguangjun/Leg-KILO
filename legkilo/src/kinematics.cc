...
void Kinematics::processing(const unitree_legged_msgs::HighState &high_state,
  ...
  */
  ...
  this->caculateFootPosVel(foot_angle, foot_angle_vel, kin_imu_meas.foot_pos_,
                           kin_imu_meas.foot_vel_);
  ...
  //     << high_state.footPosition2Body[uni_index].x << " " <<
  //     high_state.footPosition2Body[uni_index].y << " "<<
  //     high_state.footPosition2Body[uni_index].z <<  std::endl; std::cout <<
  //     "my vel: " << kin_imu_meas.foot_vel_[index][0] << " " <<
  //     kin_imu_meas.foot_vel_[index][1] << " " <<
  //     kin_imu_meas.foot_vel_[index][2] << std::endl; std::cout << "uni vel: "
  //     << high_state.footSpeed2Body[uni_index].x << " " <<
  //     high_state.footSpeed2Body[uni_index].y << " " <<
  //     high_state.footSpeed2Body[uni_index].z << std::endl << std::endl;
  // }
}

void Kinematics::caculateFootPosVel(const double (&foot_angle)[4][3],
                                    const double (&foot_angle_vel)[4][3],
                                    double (&foot_pos)[4][3],
                                    double (&foot_vel)[4][3]) {
  for (int i = 0; i < 4; ++i) {
    int lfoot = -1, ffoot = -1;
    ...
    foot_pos[i][1] =
        lfoot * d_ * c1 + lc_ * s1 * c23 + lt_ * c2 * s1 + lfoot * oy_;
    ...
    foot_vel[i][0] =
        jacb[0][1] * foot_angle_vel[i][1] + jacb[0][2] * foot_angle_vel[i][2];
    foot_vel[i][1] = jacb[1][0] * foot_angle_vel[i][0] +
                     jacb[1][1] * foot_angle_vel[i][1] +
                     jacb[1][2] * foot_angle_vel[i][2];
    foot_vel[i][2] = jacb[2][0] * foot_angle_vel[i][0] +
                     jacb[2][1] * foot_angle_vel[i][1] +
                     jacb[2][2] * foot_angle_vel[i][2];
  }
}
