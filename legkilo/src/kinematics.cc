...
void Kinematics::processing(const unitree_legged_msgs::HighState& high_state, common::KinImuMeas& kin_imu_meas){
    ...
    */
    ...
    //     std::cout << "my pos: " << kin_imu_meas.foot_pos_[index][0] << " " << kin_imu_meas.foot_pos_[index][1] << " " << kin_imu_meas.foot_pos_[index][2] << std::endl;
    //     std::cout << "uni pos: " << high_state.footPosition2Body[uni_index].x << " " << high_state.footPosition2Body[uni_index].y << " "<< high_state.footPosition2Body[uni_index].z <<  std::endl;
    //     std::cout << "my vel: " << kin_imu_meas.foot_vel_[index][0] << " " << kin_imu_meas.foot_vel_[index][1] << " " << kin_imu_meas.foot_vel_[index][2] << std::endl;
    //     std::cout << "uni vel: " << high_state.footSpeed2Body[uni_index].x << " " << high_state.footSpeed2Body[uni_index].y << " " << high_state.footSpeed2Body[uni_index].z << std::endl << std::endl;
    // }

}

void Kinematics::caculateFootPosVel(const double (&foot_angle)[4][3], const double (&foot_angle_vel)[4][3], double (&foot_pos)[4][3], double (&foot_vel)[4][3]){
    for(int i = 0; i < 4; ++i){
        int lfoot = -1, ffoot = -1;
        if(i < 2) ffoot = 1;
        if(i == 0 || i == 2) lfoot = 1;
