...
void LidarProcessing::processing(const sensor_msgs::PointCloud2::ConstPtr &msg,
...
void LidarProcessing::velodyneHandler(
  ...
  float first_point_time =
      config_.time_scale_ * cloud_pcl_raw.points.front().time;
  float last_point_time =
      config_.time_scale_ * cloud_pcl_raw.points.back().time;
  ...
  for (int i = 0; i < cloud_size; ++i) {
    if ((i % config_.filter_num_) || blindCheck(cloud_pcl_raw.points[i]))
      continue;
    ...
    added_point.curvature =
        std::round((cur_point_time - first_point_time) * 500.0f) / 500.0f;

    lidar_scan.cloud_->points.push_back(added_point);
  }
}

void LidarProcessing::ousterHander(
    const sensor_msgs::PointCloud2::ConstPtr &msg,
    common::LidarScan &lidar_scan) {
  lidar_scan.cloud_.reset(new PointCloudType());
