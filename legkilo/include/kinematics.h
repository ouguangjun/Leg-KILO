...
public:
  ...
  explicit Kinematics(const Config &config)
      : ox_(config.leg_offset_x), oy_(config.leg_offset_y),
        lc_(config.leg_calf_length), lt_(config.leg_thigh_length),
        d_(config.leg_thigh_offset),
        contacts_(std::vector<ContactDetector>(
            4, ContactDetector(config.contact_force_threshold_up,
                               config.contact_force_threshold_down))) {}

  void processing(const unitree_legged_msgs::HighState &high_state,
                  common::KinImuMeas &kin_imu_meas);

private:
  void caculateFootPosVel(const double (&foot_angle)[4][3],
