...
public:
    ...
    bool update(double val) {
        if(!in_contact_ && val > T_on_)       in_contact_ = true;
        else if(in_contact_ && val < T_off_)  in_contact_ = false;
        return in_contact_;
    }
};
...
public:
    ...
    explicit Kinematics(const Config& config)
        :   ox_(config.leg_offset_x),
            ...
            contacts_(std::vector<ContactDetector>(4, 
                ContactDetector(config.contact_force_threshold_up, config.contact_force_threshold_down)))
        {}
...
private:
    ...
    void caculateFootPosVel(const double (&foot_angle)[4][3], const double (&foot_angle_vel)[4][3], double (&foot_pos)[4][3], double (&foot_vel)[4][3]);
