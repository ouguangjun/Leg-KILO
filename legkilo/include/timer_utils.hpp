...
public:
  ...
  static void measure(const std::string &func_name, Func &&func,
                      Args &&... args) {
    getInstance().internalMeasure(func_name, std::forward<Func>(func),
...
private:
  ...
  template <typename Func, typename... Args>
  void internalMeasure(const std::string &func_name, Func &&func,
                       Args &&... args) {
  ...
  void internalLogAllAverTime() {
    ...
    for (const auto &key : keys_) {
      LOG(INFO) << key << " average time : " << average_time_map_[key].second
                << " ms";
    }
    LOG(INFO) << "======================================";
  }
